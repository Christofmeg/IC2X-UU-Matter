...
public class ModJEIPlugin implements IModPlugin {
    ...
    boolean classic = ProfileManager.selected.name.contains(ProfileManager.CLASSIC);
    boolean experimental = ProfileManager.selected.name.contains(ProfileManager.EXPERIMENTAL);
    ...
    public void register(@Nonnull IModRegistry registration) {
        ...
        if (experimental) {
            ...
            registration.addRecipeCatalyst(BlockName.te.getItemStack(TeBlock.pattern_storage),
                    ...
                    .addRecipes(
                            Arrays.asList(
                                    ...
                                    new MatterFabricatorCategory.MatterFabricatorRecipe(
                                            new ItemStack(
                                                    ItemName.crafting.getItemStack(CraftingItemType.scrap).getItem(),
                                                    scrapCount, 23),
                                            new FluidStack(FluidName.uu_matter.getInstance(), 1), "5,000"),

                                    new MatterFabricatorCategory.MatterFabricatorRecipe(
                                            new ItemStack(ItemName.crafting.getItemStack(CraftingItemType.scrap_box)
                                                    .getItem(), scrapBoxCount, 24),
                            ...
                            registration
                                    .addRecipes(
                                            Collections.singletonList(new ReplicatorCategory.ReplicatorRecipe(
                                                    new FluidStack(FluidName.uu_matter.getInstance(), mb),
                                                    item.getKey(), Utils.getCrystalMemory(item.getKey()))),
                                            ReplicatorCategory.UID);

                            registration.addRecipes(Collections.singletonList(new ScannerCategory.ScannerRecipe(
                                    item.getKey(), Utils.getCrystalMemory(item.getKey()))), ScannerCategory.UID);
            ...
            recipeTransferRegistry.addRecipeTransferHandler(
                    ...
                    MatterFabricatorCategory.UID);
            recipeTransferRegistry.addRecipeTransferHandler(ContainerPatternStorage.class, PatternStorageCategory.UID,
                    36, 1, 0, 36);
            ...
            registration.addRecipeCatalyst(BlockName.te.getItemStack(TeBlock.mass_fabricator),
                    MassFabricatorCategory.UID);

            registration.addRecipes(
                    Arrays.asList(new MassFabricatorCategory.MassFabricatorRecipe(new ItemStack(Items.AIR),
                            ItemName.misc_resource.getItemStack(MiscResourceType.matter), null)),
                    MassFabricatorCategory.UID);

            registration.addRecipes(
                    Arrays.asList(
                            new MassFabricatorCategory.MassFabricatorRecipe(new ItemStack(
                                    ItemName.crafting.getItemStack(CraftingItemType.scrap).getItem(), scrapCount, 23),
                                    ItemName.misc_resource.getItemStack(MiscResourceType.matter), "5,000"),
                            new MassFabricatorCategory.MassFabricatorRecipe(
                                    new ItemStack(ItemName.crafting.getItemStack(CraftingItemType.scrap).getItem(),
                                            scrapBoxCount, 24),
                                    ItemName.misc_resource.getItemStack(MiscResourceType.matter), "45,000")),
                    MassFabricatorCategory.UID);
    ...
    public void registerCategories(IRecipeCategoryRegistration registration) {
        ...
        if (experimental) {
            ...
            registration.addRecipeCategories(new MassFabricatorCategory(helper));
        }
